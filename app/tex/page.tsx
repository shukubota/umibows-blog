...
export default function TexPage() {
  ...
  const startCamera = async () => {
    try {
      ...
      setError(
        `Could not access camera: ${err instanceof Error ? err.message : "Unknown error"}. Please allow camera permissions and use HTTPS.`
      );
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <header className="space-y-2">
          ...
          {typeof window !== "undefined" && !window.location.protocol.startsWith("https") && (
            <div className="mt-2 p-3 bg-yellow-900/50 border border-yellow-500/30 rounded-lg text-yellow-200 text-sm">
              <strong>Note:</strong> Camera access requires HTTPS. Run{" "}
              <code className="bg-yellow-800/50 px-1 rounded">npm run dev:https</code> for camera
              functionality.
            </div>
          )}
        </header>
        ...
        <section className="bg-gray-800 p-4 rounded-xl border border-gray-700">
          ...
          <p className="text-sm text-gray-400 text-center">
            Draw mathematical expressions above and click &quot;Convert to TeX&quot; to generate
            LaTeX code
          </p>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <section className="space-y-4">
            ...
            <div className="w-full h-64 p-8 bg-white text-gray-900 rounded-xl flex items-center justify-center overflow-auto shadow-lg">
              <div dangerouslySetInnerHTML={{ __html: html }} className="text-2xl" />
            </div>
            ...
            <div className="p-4 bg-gray-800/50 rounded-lg text-sm text-gray-400 space-y-2 border border-gray-700">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => setInput("\\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}")}
                  className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs transition-colors"
                >
                  Quadratic
                </button>
                <button
                  onClick={() => setInput("\\int_{-\\infty}^{\\infty} e^{-x^2} dx = \\sqrt{\\pi}")}
                  className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs transition-colors"
                >
                  Gaussian Integral
                </button>
                <button
                  onClick={() =>
                    setInput("\\sum_{n=1}^{\\infty} \\frac{1}{n^2} = \\frac{\\pi^2}{6}")
                  }
                  className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs transition-colors"
                >
                  Basel Problem
