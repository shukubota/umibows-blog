...
```bash
gcloud iam workload-identity-pools providers create-oidc vercel-provider --workload-identity-pool vercel-pool --location global --issuer-uri "https://oidc.vercel.com" --attribute-mapping "google.subject=assertion.sub,attribute.environment=assertion.environment" --project $GOOGLE_CLOUD_PROJECT_ID
```
