...
const HONOR: Record<string, number> = {
  ton: 27, east: 27, e: 27, "東": 27,
  nan: 28, south: 28, s: 28, "南": 28,
  sha: 29, shaa: 29, west: 29, w: 29, "西": 29,
  pei: 30, north: 30, n: 30, "北": 30,
  haku: 31, white: 31, "白": 31,
  hatsu: 32, green: 32, "發": 32, "発": 32,
  chun: 33, red: 33, "中": 33,
};
...
function shantenNormal(src: number[]): number {
  ...
  function rec(i: number, m: number, t: number, h: boolean) {
    ...
    if (c[i] >= 3 && m + t < 4) { c[i] -= 3; rec(i, m + 1, t, h); c[i] += 3; }
    if (i < 27 && i % 9 <= 6 && c[i] && c[i + 1] && c[i + 2] && m + t < 4) {
      c[i]--; c[i + 1]--; c[i + 2]--; rec(i, m + 1, t, h); c[i]++; c[i + 1]++; c[i + 2]++;
    }
    if (c[i] >= 2 && !h) { c[i] -= 2; rec(i, m, t, true); c[i] += 2; }
    if (c[i] >= 2 && m + t < 4) { c[i] -= 2; rec(i, m, t + 1, h); c[i] += 2; }
    if (i < 27 && i % 9 <= 7 && c[i] && c[i + 1] && m + t < 4) {
      c[i]--; c[i + 1]--; rec(i, m, t + 1, h); c[i]++; c[i + 1]++;
    }
    if (i < 27 && i % 9 <= 6 && c[i] && c[i + 2] && m + t < 4) {
      c[i]--; c[i + 2]--; rec(i, m, t + 1, h); c[i]++; c[i + 2]++;
    }
    c[i]--; rec(i, m, t, h); c[i]++;
  }
  rec(0, 0, 0, false);
...
function ukeire(c: number[]): { base: number; total: number; tiles: number[] } {
  ...
  for (let t = 0; t < 34; t++) {
    if (c[t] >= 4) continue;
    c[t]++;
    if (shanten(c) < base) { tiles.push(t); total += 4 - (c[t] - 1); }
    c[t]--;
  }
...
export function createServer(): McpServer {
  ...
  registerAppTool(
    ...
    "show_mahjong_hand",
    {
      ...
      inputSchema: {
        tiles: z.array(z.string()).describe("牌コードの配列。例: ['1p','2p','3p','3m','4m','5m','chun','chun','hatsu','hatsu','ton','nan','sha']"),
      },
    ...
    async ({ tiles }): Promise<CallToolResult> => {
      ...
      for (const code of tiles) {
        ...
        if (idx === null) { unknown.push(code); continue; }
        counts[idx]++;
        handIdx.push(idx);
      }
      ...
      if (total !== 13 && total !== 14) {
        ...
        mode = "improve";
        const uk = ukeire(counts);
        recommend = uk.tiles.map((t) => ({ index: t, name: NAMES[t], shanten: sh - 1, ukeire: 4 - counts[t] }));
      }
      ...
      else if (mode === "complete") text = `和了形です（シャンテン -1）。手牌: ${structuredContent.names.join(" ")}`;
