...
const LorenzAttractor: React.FC = () => {
    ...
    const rk4 = useCallback((p: Point, dt: number): Point => {
        ...
        const k1 = f(p);
        const k2 = f({ x: p.x + k1.x * dt * 0.5, y: p.y + k1.y * dt * 0.5, z: p.z + k1.z * dt * 0.5 });
        const k3 = f({ x: p.x + k2.x * dt * 0.5, y: p.y + k2.y * dt * 0.5, z: p.z + k2.z * dt * 0.5 });
    ...
    const draw = useCallback(() => {
        ...
        if (pointsRef.current.length > 0) {
            ...
            ctx.moveTo(centerX + currentPosRef.current.x * scale, centerY - currentPosRef.current.z * scale);
        }
        ...
        <div className="relative w-screen h-screen bg-black overflow-hidden font-sans">
            ...
            <div className="absolute top-4 right-4 p-4 bg-gray-900/30 text-white rounded-xl border border-white/5 backdrop-blur-sm pointer-events-none select-none">
                <div className="font-mono text-sm opacity-80 space-y-1">
                    <div><span className="text-green-400">dx/dt</span> = σ(y - x)</div>
                    <div><span className="text-blue-400">dy/dt</span> = x(ρ - z) - y</div>
                    <div><span className="text-pink-400">dz/dt</span> = xy - βz</div>
                </div>
            </div>
            ...
            {/* Controls Panel */}
            <div
                className={`absolute top-4 left-4 p-6 w-80 bg-gray-900/90 backdrop-blur-md text-white rounded-xl border border-white/10 shadow-2xl transition-all duration-300 transform origin-top-left ${isControlsOpen ? 'scale-100 opacity-100 translate-y-12' : 'scale-95 opacity-0 pointer-events-none translate-y-0'
                    }`}
            >
